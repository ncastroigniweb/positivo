...
class Reports_model extends CI_Model
{
    ...
    public function getStaffDailySales($user_id, $year, $month, $warehouse_id = NULL)
    {
        ...
        if($this->sma->is_admin($user_id) || $this->sma->is_cashier($user_id)){
            $myQuery .= " created_by = {$user_id} AND DATE_FORMAT( date,  '%Y-%m' ) =  '{$year}-{$month}'
            GROUP BY DATE_FORMAT( date,  '%e' )";
        } else {
            $myQuery .= " id_waiter = {$user_id} AND DATE_FORMAT( date,  '%Y-%m' ) =  '{$year}-{$month}'
            GROUP BY DATE_FORMAT( date,  '%e' )";
        }
    ...
    public function getStaffMonthlySales($user_id, $year, $warehouse_id = NULL)
    {
        ...
        if($this->sma->is_admin($user_id) || $this->sma->is_cashier($user_id)){
            ...
            $myQuery .= " id_waiter = {$user_id} AND DATE_FORMAT( date,  '%Y' ) =  '{$year}'
            GROUP BY date_format( date, '%c' ) ORDER BY date_format( date, '%c' ) ASC";
        }
    ...
    {
        $this->db->select('count(id) as total, sum(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax, SUM(COALESCE(total_discount, 0)) as discount, SUM( COALESCE( order_tip, 0 ) ) AS tip', FALSE)
    ...
    public function getTotalOtherProfits($start, $end, $warehouse_id = NULL)
    {
        $this->db->select('count(id) as total, sum(COALESCE(amount, 0)) as total_amount', FALSE)
            ->where('date BETWEEN ' . $start . ' and ' . $end);
        if ($warehouse_id) {
    ...
    public function getBestSeller($start_date, $end_date, $warehouse_id = NULL, $limit = 10)
    {
        $this->db
