...
class Products_model extends CI_Model
{
    ...
    public function getSuspendedItem($id)
    {
        $q = $this->db->get_where('sma_suspended_items', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    /**
     * @param $text condition to search products
     * @return list with products
     */
    public function getProductsByText($text)
    {
        //$this->db->select('products.name,products.price, products.code, products.id ,categories.id, categories.name as cat_name');
        $this->db->select('products.* ,categories.id as cat_id');
        $this->db->from('products');
        $this->db->like('products.name', $text);
        $this->db->where('products.status', 1);
        $this->db->where('categories.status', 1);
        $this->db->join('categories', 'categories.id = products.subcategory_id', 'left');
        $q = $this->db->get();
    ...
    public function getParentCategories()
    {
        ...
        $q = $this->db->get_where('categories', array('parent_id' => 0, 'status' => 1));
    ...
    public function getSubCategoryByParent($parent = null)
    {
        
        if ($parent){
            $q = $this->db->get_where('categories', array('parent_id' => $parent, 'status' => 1));
    ...
    public function getSubCategoriesByParents($parents =array())
    {
        ...
        if ($parents){
            ...
            foreach($parents as $parent){
                $data[$parent] = $this->getSubCategoryByParent($parent);
    ...
    // Return acomplete list of dispatched items of current user
    public function get_dispatched_list($limit = 0, $product_category = null, $permissions = null){
        
        $data = array();
        $this->db->order_by('date_dispatched','DESC');
        ...
        // where admin
        if ($permissions) {
            $where = array('product_category' => $product_category);
        } else {
            $where = array('user_id' => $this->session->userdata('user_id'));
        }
        
        $q = $this->db->get_where('dispatched_items', $where);
        if ($q->num_rows() > 0) {
            foreach ($q->result() as $row) {
                $data[] = $row;                        
            }
    ...
    public function dispatchItem(array $array){
        ...
        foreach($default as $key => $value){
            $default[$key] = (isset($array[$key])) ? $array[$key] : $value;
        }
        
        $default['date_dispatched'] = date("Y-m-d H:i:s");
        $this->db->insert('dispatched_items',$default);
        
    }
    /*Catidad según item */
    public function getProductCant($code)
    {
        $q = $this->db->get_where('products', array('code' => $code), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
    ...
    /** */
    //Consulta de productos tipo combos
    public function getProductsCombos()
    {
        $this->db
            ->select('id')
            ->from('products')
    ...
    //Obtener codigos de los items del un producto tipo combo
    public function getItemCombo($id)
    {
        $this->db
            ->select('item_code')
            ->from('combo_items')
    ...
    //Ajuste de cantidades en productos tipo combo cada vez que se actualiza un item del combo
    public function updateProductsCombos()
    {
        //Obtener lo productos tipo combos
        $products=$this->getProductsCombos();
        $items=array();
        $cantItem=array();

        //Contamos los productos
        $countProd= count($products);  
        //Recorrer todos los productos junto con sus items      
        for ($i=0; $i < $countProd ; $i++) {
            $items = $this->getItemCombo($products[$i]->id);        
            foreach ($items as $item) {
                $quantity=$this->getProductCant($item->item_code);
                array_push($cantItem, $quantity->quantity);
            }
            //Ordenar los items de los combos de menor a mayor
            asort($cantItem);
            $cantVal = current($cantItem);
            //Actualizar la cantidad del combo según el item menor que lo compone
            $this->db->update('products', array('quantity' => $cantVal), array('id' => $products[$i]->id));
            unset($cantItem); 
            $cantItem=array(); 
        }        
        return TRUE;
