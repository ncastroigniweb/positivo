...
class Pos_model extends CI_Model
{
    ...
    public function getUserBill($id)
    {
...
//        $group->name == "waiter" || $group->name == "mozo"
    ...
    //update N° reference Dian
    public function updateSaleDianId($id,$reference_dian)
    {
        $q = $this->db->update('sales', array('reference_dian' => $reference_dian), array('id' => $id));
        if ($q)
        {
            return TRUE;
        }
    ...
    //update sales trans ID
    public function updateSalesTransId($transaccion_id,$reference_dian)
    {
        $q = $this->db->update('sales', array('transaccion_id' => $transaccion_id), array('reference_dian' => $reference_dian));
        if ($q)
        {
            return TRUE;
        }
    ...
    public function countSalesDay()
    {
        $this->db
            ->select('*')
            ->from('sales')
            ->where('date >=', date("Y-m-d").' 06:00:00')
            ->where('date <',  date("Y-m-d").' 23:00:00')
            ->where('reference_dian=',NULL);
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getProductId($sale_id)
    {
        $this->db->select('product_id,product_type', FALSE)
    ...
    //Consulta de cantidad de un productos según su código
    public function getProductByCode2($code)
    {
    ...
    //actualización cantidad del producto
    public function updateProductQuantity2($id)
    {
        $codes = $this->getItemCombo($id);

        
        $cantid= array();
        foreach ($codes as $code) {
           array_push($cantid, $this->getProductByCode2($code->item_code)); 
        }
        asort($cantid);
        $quantity=current($cantid);

        //combo_items
        if ($this->db->update('products', array('quantity' => $quantity->quantity), array('id' => $id))) {
            return TRUE;
        }
