...
class system_settings extends MY_Controller
{
    ...
    function index()
    {
        ...
        $this->form_validation->set_rules('show_reference', lang('show_reference'), 'trim|required');
        ...
        $this->form_validation->set_rules('delay_product', lang('delay_product'), 'trim|required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($this->input->post('date_format_chef_hour')){
                $date_chef .= "H";
            }
            if($this->input->post('date_format_chef_min')){
                $date_chef .= "i";
            }
            if($this->input->post('date_format_chef_seg')){
                $date_chef .= "s";
            }
            ...
            foreach ($days as $day) {
                if($this->input->post(strtolower($day . '_premium_price'))){
                    if(!empty($days_premium_price)){
                        $days_premium_price .= ',';
                    }
                    $days_premium_price .= $day;
                }
            }
            ...
            $data = array('site_name' => DEMO ? 'Stock Manager Advance' : $this->input->post('site_name'),
                ...
                'show_reference' => trim($this->input->post('show_reference')),
                ...
                'date_format_chef' => $date_chef,
                'delay_product' => $this->input->post('delay_product'),
                'send_to' => $this->input->post('send_to'),
                'days_premium_price' => $days_premium_price,
                'status_premium_price' => $this->input->post('status_premium_price'),
                'sale_tax_method' => $this->input->post('sale_tax_method'),
            );
    ...
    function edit_category($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'top_list' => $this->input->post('top_list')
                );
    ...
    function tip_rates()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('tip_rates')));
        $meta = array('page_title' => lang('tip_rates'), 'bc' => $bc);
        $this->page_construct('settings/tip_rates', $meta, $this->data);
    }
    
    function getTipRates()
    {
        ...
        $this->datatables
            ->select("id, name, code, rate, type")
            ->from("tip_rates")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . site_url('system_settings/edit_tip_rate/$1') . "' class='tip' title='" . lang("edit_tip_rate") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_tip_rate") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('system_settings/delete_tip_rate/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        //->unset_column('id');

        echo $this->datatables->generate();
    }

    function add_tip_rate()
    {
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(-1);

        $this->form_validation->set_rules('name', lang("name"), 'trim|is_unique[tip_rates.name]|required');
        $this->form_validation->set_rules('type', lang("type"), 'required');
        $this->form_validation->set_rules('rate', lang("tip_rate"), 'required|numeric');
        ...
        } elseif ($this->input->post('add_tip_rate')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect("system_settings/tip_rates");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addTipRate($data)) {
            $this->session->set_flashdata('message', lang("tip_rate_added"));
            redirect("system_settings/tip_rates");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_tip_rate', $this->data);
        }
    }

    function edit_tip_rate($id = NULL)
    {

        $this->form_validation->set_rules('name', lang("name"), 'trim|required');
        $tip_details = $this->settings_model->getTipRateByID($id);
        if ($this->input->post('name') != $tip_details->name) {
            $this->form_validation->set_rules('name', lang("name"), 'is_unique[tip_rates.name]');
        }
        $this->form_validation->set_rules('type', lang("type"), 'required');
        $this->form_validation->set_rules('rate', lang("tip_rate"), 'required|numeric');
        ...
        } elseif ($this->input->post('edit_tip_rate')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect("system_settings/tip_rates");
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateTipRate($id, $data)) { //check to see if we are updateing the customer
            $this->session->set_flashdata('message', lang("tip_rate_updated"));
            redirect("system_settings/tip_rates");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['tip_rate'] = $this->settings_model->getTipRateByID($id);

            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_tip_rate', $this->data);
        }
    }

    function delete_tip_rate($id = NULL)
    {
        if ($this->settings_model->deleteTipRate($id)) {
            echo lang("tip_rate_deleted");
        }
    }

    function tip_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteTipRate($id);
                    }
                    $this->session->set_flashdata('message', lang("tip_rates_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('tip_rates'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('name'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('code'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('tip_rate'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('type'));

                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $tip = $this->settings_model->getTipRateByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $tip->name);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $tip->code);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $tip->rate);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, ($tip->type == 1) ? lang('percentage') : lang('fixed'));
                        $row++;
                    }
                    $this->excel->getActiveSheet()->getColumnDimension('A')->setWidth(20);
                    $this->excel->getActiveSheet()->getColumnDimension('C')->setWidth(20);
                    $this->excel->getActiveSheet()->getColumnDimension('D')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $filename = 'tip_rates_' . date('Y_m_d_H_i_s');
                    if ($this->input->post('form_action') == 'export_pdf') {
                        $styleArray = array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)));
                        $this->excel->getDefaultStyle()->applyFromArray($styleArray);
                        $this->excel->getActiveSheet()->getPageSetup()->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
