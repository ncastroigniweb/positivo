...
class Products extends MY_Controller
{
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'premium_price' => $this->input->post('premium_price') ? $this->input->post('premium_price') : NULL,
            );
            ...
            if ($this->input->post('type') == 'standard') 
            {
                ...
                $warehouse_qty = NULL;
                $product_attributes = NULL;
               // $wh_total_quantity = $this->input->post('wh_qty_1');
            }
            /**Variable para item menor */
            $itemMenor = array();
            /****/
            if ($this->input->post('type') == 'service') {
                ...
                for ($r = 0; $r <= $c; $r++) {
                    ...
                    /* Validación item con menor cantidad */
                        $itemProd= $this->products_model->getProductCant($_POST['combo_item_code'][$r]);
                        array_push($itemMenor, $itemProd->quantity);
                        

                      
                    }
                    asort($itemMenor);
                    $wh_total_quantity = current($itemMenor);
                    /****/
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run('products/add') == true) {
            ...
            if ($this->input->post('type') == 'service') {
                ...
                /* Validación de precio versus valores de los items que lo componen                
                if ($this->sma->formatDecimal($total_price) != $this->sma->formatDecimal($this->input->post('price'))) {
                    $this->form_validation->set_rules('combo_price', 'combo_price', 'required');
                    $this->form_validation->set_message('required', lang('pprice_not_match_ciprice'));
                }*/
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('name', 'code', 'barcode_symbology', 'brand', 'category_code', 'unit', 'sale_unit', 'purchase_unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'subcategory_code', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6', 'product_details');
            ...
            $ikeys = array('code', 'barcode_symbology', 'name', 'brand', 'category_id', 'unit', 'sale_unit', 'purchase_unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'subcategory_id', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6', 'product_details');

            $items = array();
            foreach (array_map(null, $pr_code, $bs, $pr_name, $pr_brand, $pr_cat, $pr_unit, $sale_units, $purcahse_units, $pr_cost, $pr_price, $pr_aq, $pr_tax, $tax_method, $pr_subcat, $pr_variants, $cf1, $cf2, $cf3, $cf4, $cf5, $cf6, $pr_details) as $ikey => $value) {
        ...
        if ($this->form_validation->run() == true && $prs = $this->products_model->add_products($items)) {
            ...
            foreach ($items as $valuepro){
                $prod = $this->products_model->getProductByCode($valuepro['code']);
                $this->products_model->addQuantity($prod->id,1,floatval(50),null);
            }
    ...
    function add_adjustment()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // if you want to change quantity stock for all products discomment code
//            $all_products = $this->products_model->getAllProducts();
//            $_POST['product_id'] = null;
//            foreach ($all_products as $all_product) {
//                $_POST['product_id'][] = $all_product->id;
//            }

            // if you want to change quantity stock for some products discomment and put ids products
//            $_POST['product_id'] = null;
...
//                $quantity = 10000;
        ...
        if ($this->form_validation->run() == true && $this->products_model->addAdjustment($data, $products)) {
            //Ajuste de cantidades en todos los productos tipo combos
            $this->products_model->updateProductsCombos();
            //
            ...
            //$this->data['codes'] = $this->products_model->updateProductsCombos();
