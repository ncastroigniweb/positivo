...
class Reports extends MY_Controller
{
    ...
    {   
        ...
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        $user = $this->input->get('user') ? $this->input->get('user') : 0;
        ...
        $admin = $this->sma->is_admin($user);
        $cashier = $this->sma->is_cashier($user);
        ...
        if ($pdf || $xls) {
            ...
            if ($user) {
                if($admin || $cashier){
                    ...
                    $this->db->where('sales.id_waiter', $user);
                }
            }
            if ($product) {
            ...
            if ($user) {
                ...
                if($admin || $cashier){
                    ...
                    $this->datatables->where('sales.id_waiter', $user);
                }
            }
            if ($product) {
    ...
    function profit_loss($start_date = NULL, $end_date = NULL)
    {
        ...
        $this->data['total_other_profits'] = $this->reports_model->getTotalOtherProfits($start, $end);
        $this->data['total_products_tax'] = $this->sma->get_products_tax($start, $end);
    ...
    function getRrgisterlogs($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, closed_at, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name, ' (', users.email, ')') as user, cash_in_hand, total_cc_slips, total_cheques, total_cash, total_cc_slips_submitted, total_cheques_submitted,total_cash_submitted, json_data,note", FALSE)
                ->from("pos_register")
            ...
            $this->load->library('datatables');
            $this->datatables
            ->select("date, closed_at, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name, '<br>', " . $this->db->dbprefix('users') . ".email) as user, cash_in_hand, CONCAT(total_cc_slips, ' (', total_cc_slips_submitted, ')'), CONCAT(total_cheques, ' (', total_cheques_submitted, ')'), CONCAT(total_cash, ' (', total_cash_submitted, ')'), json_data,note,", FALSE)
                ->from("pos_register")
                ->join('users', 'users.id=pos_register.user_id', 'left');
    ...
    public function other_profits($id = null)
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['categories'] = $this->reports_model->getOtherProfitsCategories();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('other_profits')));
        $meta = array('page_title' => lang('other_profits'), 'bc' => $bc);
        $this->page_construct('reports/other_profits', $meta, $this->data);
    }

    public function getOtherProfitsReport($pdf = NULL, $xls = NULL)
    {
//        $this->sma->checkPermissions('expenses');
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, reference, {$this->db->dbprefix('other_profits_categories')}.name as category, amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment, {$this->db->dbprefix('other_profits')}.id as id", false)
            ->from('other_profits')
            ...
            if (!empty($data)) {
                ...
                $this->load->library('excel');
                $this->excel->setActiveSheetIndex(0);
                $this->excel->getActiveSheet()->setTitle(lang('other_profits_report'));
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('reference_no'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('category'));
                $this->excel->getActiveSheet()->SetCellValue('D1', lang('amount'));
            ...
            $this->load->library('datatables');
            $this->datatables
            ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference, {$this->db->dbprefix('other_profits_categories')}.name as category, amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment, {$this->db->dbprefix('other_profits')}.id as id", false)
            ->from('other_profits')
            ->join('users', 'users.id=other_profits.created_by', 'left')
            ->join('other_profits_categories', 'other_profits_categories.id=other_profits.category_id', 'left')
            ->group_by('other_profits.id');
    ...
    public function guests($id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('guests')));
        $meta = array('page_title' => lang('guests'), 'bc' => $bc);
        $this->page_construct('reports/guests', $meta, $this->data);
    }

    public function getGuestsReport($pdf = NULL, $xls = NULL)
    {

        $reference_no = $this->input->get('reference_no') ? $this->input->get('reference_no') : NULL;
        $table = $this->input->get('table') ? $this->input->get('table') : NULL;
        $guests = $this->input->get('guests') ? $this->input->get('guests') : NULL;
        $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
        $end_date = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, reference_no, {$this->db->dbprefix('tables')}.name as table_name, {$this->db->dbprefix('sales')}.guests", false)
                ->from('sales')
                ->join('tables', 'sales.id_table=tables.id', 'inner');
            ...
            if ($guests) {
                $this->db->where("{$this->db->dbprefix('sales')}.guests", $guests);
            }
            if ($start_date) {
                $this->db->where('date BETWEEN "' . $start_date . '" and "' . $end_date . '"');
            }
            ...
            if (!empty($data)) {
                ...
                $this->load->library('excel');
                $this->excel->setActiveSheetIndex(0);
                $this->excel->getActiveSheet()->setTitle(lang('guests_report'));
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('reference_no'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('table'));
                $this->excel->getActiveSheet()->SetCellValue('D1', lang('guests'));

                $row = 2; $total = 0; 
                foreach ($data as $data_row) {
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->reference_no);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->table_name);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->guests);
                    $total += $data_row->guests;
                    $row++;
                }
                $this->excel->getActiveSheet()->getStyle("C" . $row)->getBorders()
                    ->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
                $this->excel->getActiveSheet()->SetCellValue('C' . $row, ($row - 2));
            ...
            if ($reference_no) {
                $this->datatables->like('reference_no', $reference_no, 'both');
            }
            if ($table) {
                $this->db->where('id_table', $table);
            }
