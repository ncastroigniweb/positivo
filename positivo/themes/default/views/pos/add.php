...
<div id="wrapper">
    <header id="header" class="navbar navbar-pos">
        <div class="container">
            ...
            <div class="header-nav">
                ...
                <ul class="nav navbar-nav pull-right">
                    ...
                    <li class="dropdown">
                        <a class="btn bblue pos-tip" id="daily_details" title="<span><?=lang('daily_details')?></span>" data-placement="bottom" data-html="true" href="<?=site_url('pos/daily_details')?>" data-toggle="modal" data-target="#myModal">
                    ...
                    <li class="dropdown">
                        <a class="btn borange pos-tip" id="show_expense" title="<span><?=lang('expenses')?></span>" data-placement="bottom" data-html="true" href="<?=site_url('pos/view_expenses')?>" data-toggle="modal" data-target="#myModal">
                    ...
                    <li class="dropdown">
                        <div class="onoffswitch-tips">
                            ...
                            $data = array(
                                    ...
                                    'id'            => 'active_tips',
                                    'value'         => 1,
                                    'checked'       => set_value('active_tips', $this->sma->get_activated_tip()),
                                    'class'         => 'onoffswitch-tips-checkbox'
                            );
                            ...
                            <label class="onoffswitch-tips-label" for="active_tips">
                                <span class="onoffswitch-tips-inner"></span>
                                <span class="onoffswitch-tips-switch"></span>
                            </label>
                        </div>
                    </li>
                    <?php }?>
                    ...
                    if( $Owner ||$Admin || $GP['restaurants-index']){
                        ...
                        <div id="print">
                            ...
                            <div id="left-bottom">
                                <table id="totalTable"
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding: 5px 10px;"><?=lang('discount');?>
                                            <?php if ($Owner || $this->sma->is_cashier() || $Admin || $this->session->userdata('allow_discount')) { ?>
                                ...
                                <input name="order_tax" type="hidden" value="<?= ($Settings->default_tax_rate2 != 0) ? $Settings->default_tax_rate2 : ($suspend_sale ? $suspend_sale->order_tax_id : $Settings->default_tax_rate2);?>" id="postax2">
                                <input name="discount" type="hidden" value="<?=$suspend_sale ? $suspend_sale->order_discount_id : '';?>" id="posdiscount">
                                <input name="order_tip" type="hidden" value="<?=$suspend_sale ? $suspend_sale->order_tip_id : 0;?>" id="postip">
                    ...
                    <div id="cp">
                        <div id="cpinner">
                            <div class="quick-menu">
                                <div id="proContainer">
                                    <div id="ajaxproducts">
                                        <div id="item-list" style="overflow: auto">
                                            <?php echo $products; ?>
                                        </div>
...
?>
                    <div id="container-fixed-tax" class="hide" style="margin-top: 10px;">
                        <input type="text" class="form-control pos-input-tip" id="fixed_tax">
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i
                        class="fa fa-2x">&times;</i></button>
                <h4 class="modal-title" id="tipModalLabel"><?=lang('edit_order_tip');?></h4>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <?=lang("order_tip", "order_tip_input");?>
<?php
	$tips_tr[""] = "";

	foreach ($tip_rates as $tip){
	    $tips_tr[$tip->id] = $tip->name;
	}
	echo form_dropdown('order_tip_input', $tips_tr, "", 'id="order_tip_input" class="form-control pos-input-tip" style="width:100%;"');
?>
                    <div id="container-fixed-tip" class="hide" style="margin-top: 10px;">
                        <input type="text" class="form-control pos-input-tip" id="fixed_tip">
                    </div>
                </div>

            </div>
            <div class="modal-footer">
                <button type="button" id="updateOrderTip" class="btn btn-primary"><?=lang('update')?></button>
            </div>
        </div>
    </div>
</div>
...
<div id="bill_tbl">
    ...
    <table id="bill-table" width="99%" class="prT table table-striped" style="margin-bottom:0;"></table>
    <table id="bill-total-table" class="prT table" style="margin-bottom:0;" width="99%"></table>
</div>
...
var lang = {unexpected_value: '<?=lang('unexpected_value');?>', select_above: '<?=lang('select_above');?>', r_u_sure: '<?=lang('r_u_sure');?>', bill: '<?=lang('bill');?>', order: '<?=lang('order');?>', alert_x_delete_sale: '<?=lang('alert_x_delete_sale');?>'};
</script>
...
<script type="text/javascript">
    var product_variant = 0, shipping = 0, p_page = 0, per_page = 0, tcp = "<?=$tcp?>", pro_limit = <?= $pos_settings->pro_limit; ?>,
        ...
        product_tax = 0, invoice_tax = 0, product_discount = 0, order_discount = 0, total_discount = 0, total = 0, total_paid = 0, grand_total = 0, tip = 0,
        KB = <?=$pos_settings->keyboard?>, tax_rates =<?php echo json_encode($tax_rates); ?>, tip_rates =<?php echo json_encode($tip_rates); ?>;
    var protect_delete = <?php if (!$Owner && !$Admin) {echo $pos_settings->pin_code ? '1' : '0';} else {echo '0';} ?>
    //var audio_success = new Audio('<?=$assets?>sounds/sound2.mp3');
    //var audio_error = new Audio('<?=$assets?>sounds/sound3.mp3');
    var lang_total = '<?=lang('total');?>', lang_subtotal = '<?=lang('subtotal');?>', lang_items = '<?=lang('items');?>', lang_discount = '<?=lang('discount');?>', lang_tax2 = '<?=lang('order_tax');?>', lang_total_payable = '<?=lang('total_payable');?>', lang_tip = '<?=lang('tip_sugested');?>', lang_saved_sale = '<?=lang('saved_sale');?>';
    var java_applet = <?=$pos_settings->java_applet?>, order_data = '', bill_data = '';
    var default_cc_name = 'CC', default_no_cc='000000000';

    $('#pcc_holder_1').val(default_cc_name);
    $('#pcc_no_1').val(default_no_cc);
    $('#cc_no_val_1').val($('#pcc_no_1').val());
    $('#cc_holder_val_1').val($('#pcc_holder_1').val());
...
<?php if ($this->session->userdata('remove_posls')) {?>
        ...
        widthFunctions();
        <?php if ($suspend_sale) {?>
        localStorage.setItem('postax2', '<?=($Settings->default_tax_rate2 != 0) ? $Settings->default_tax_rate2 : ($suspend_sale ? $suspend_sale->order_tax_id : $Settings->default_tax_rate2);?>');
        ...
        localStorage.setItem('poslangcustomer', '<?=$suspend_sale->lang_customer;?>');
        localStorage.setItem('posbiller', '<?=$suspend_sale->biller_id;?>');
        localStorage.setItem('postable', '<?=$suspend_sale->id_table;?>');
        localStorage.setItem('postablename', '<?=$suspend_sale->table_name;?>');
        localStorage.setItem('postablelang', '<?=$suspend_sale->table_lang;?>');
        localStorage.setItem('posbilltitle', '<?=$suspend_sale->bill_title;?>');
        localStorage.setItem('posmessagebill', '<?=$suspend_sale->message_bill;?>');
        localStorage.setItem('poswaiter', '<?=$suspend_sale->waiter_name;?>');
        localStorage.setItem('poslangwaiter', '<?=$suspend_sale->lang_waiter_name;?>');
        localStorage.setItem('posinfobiller', '<?=$suspend_sale->biller;?>');
        localStorage.setItem('posbillertel', '<?=$suspend_sale->biller_tel;?>');
        localStorage.setItem('poslogo', '<?=$suspend_sale->biller_logo;?>');
        localStorage.setItem('order_tip', '<?=$suspend_sale->order_tip_id;?>');
        <?php }
        ?>
        localStorage.setItem('posinvoicefooter', '<?= str_replace("<br />", "<br>\\", nl2br(htmlentities($biller->invoice_footer)))?>');
<?php if ($this->input->get('customer')) {?>
        ...
        if (!localStorage.getItem('order_tip')) {
            localStorage.setItem('order_tip', 0);
        }
        ...
        $('#postable').val(localStorage.getItem('postable'));
        ...
        $('#payment').click(function () {
            <?php if ($sid) {?>
            suspend = $('<span></span>');
            suspend.html('<input type="hidden" name="delete_id" value="<?php echo $sid; ?>" /><input type="hidden" name="id_suspended_sale" value="<?php echo $sid; ?>" />');
            suspend.appendTo("#hidesuspend");
            <?php }
            ?>
            var twt = 0;
            <?php if ( $Settings->sale_tax_method == 0) { ?>
                twt = formatDecimal((total + tip) - order_discount);
            <?php } else {?>
                twt = formatDecimal((total + invoice_tax + tip) - order_discount);
            <?php } ?>
        ...
        $(document).on('click', '.addButton', function () {
            if (pa <= 5) {
                ...
                $('#pcc_holder_' + pa).val(default_cc_name);
                $('#pcc_no_' + pa).val(default_no_cc);
                $('#cc_no_val_' + pa).val($('#pcc_no_' + pa).val());
                $('#cc_holder_val_' + pa).val($('#pcc_holder_' + pa).val());
        ...
        $(document).on('click', '#submit-sale', function () {
            ...
            if (total_paid == grand_total){
            ...
            } else if (total_paid == 0 || total_paid < grand_total) {
                ...
                <?php } else { ?>
                    bootbox.alert("<?= lang('paid_l_t_payable_by_cashier'); ?> ");
                <?php } ?>
                return false;
            } else {
                $('#pos_note').val(localStorage.getItem('posnote'));
...
<div id="ajaxCall"><i class="fa fa-spinner fa-pulse"></i></div>
     ...
     <?php echo "<script>console.log(".json_encode($this->session->userdata('id_venta')).")</script>"?>;
