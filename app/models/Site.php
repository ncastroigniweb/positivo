...
class Site extends CI_Model
{
    ...
    public function getAllTipRates()
    {
        $q = $this->db->get('tip_rates');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getTipRateByID($id)
    {
        $q = $this->db->get_where('tip_rates', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getReference($field, $no_reference = false) {
        ...
        if ($q->num_rows() > 0) {
            ...
            $con_number = ($no_reference) ? $ref->{$field} - 1 : $ref->{$field} ;

            if ($this->Settings->reference_format == 1) {
                $ref_no .= date('Y') . "/" . sprintf("%04s", $con_number);
            } elseif ($this->Settings->reference_format == 2) {
                $ref_no .= date('Y') . "/" . date('m') . "/" . sprintf("%04s", $con_number);
            } elseif ($this->Settings->reference_format == 3) {
                $ref_no .= sprintf("%04s", $con_number);
            } else {
                ...
                foreach ($combo_items as $combo_item) {
                    if ($combo_item->type == 'standard') {
                        if (isset($citems['p' . $combo_item->id . 'o' . $item['option_id']])) {
                            ...
                            $cproduct = array('product_id' => $combo_item->id, 'product_name' => $cpr->name, 'product_type' => $combo_item->type, 'quantity' => ($combo_item->qty*$item['quantity']), 'net_unit_price' => $net_unit_price, 'unit_price' => $unit_price, 'warehouse_id' => $item['warehouse_id'], 'item_tax' => (!empty($item_tax) ? $item_tax : NULL), 'tax_rate_id' => (!empty($cpr->tax_rate) ? $cpr->tax_rate : NULL), 'tax' => (!empty($cpr_tax) ? ($cpr_tax->type == 1 ? $cpr_tax->rate.'%' : $cpr_tax->rate) : NULL), 'option_id' => NULL, 'product_unit_id' => $cpr->unit);
